...
describe('TemplateApp - 401 자동 리다이렉트 (Issue #301)', () => {
  ...
  it('user 템플릿의 public route 초기화에서는 auth preload를 건너뛰어야 한다', async () => {
    Object.defineProperty(window, 'location', {
      ...
      if (url.includes('/routes.json')) {
        return Promise.resolve({
          ...
          json: () => Promise.resolve({
            success: true,
            data: { routes: [{ path: '/', layout: 'home', auth_required: false }] },
          }),
        });
      }
    ...
    const preloadSpy = vi.spyOn(authManager, 'preloadAuth').mockResolvedValue(null as any);
  ...
  it('user 템플릿의 auth_required route 초기화에서는 auth preload를 유지해야 한다', async () => {
    Object.defineProperty(window, 'location', {
      value: { href: '', pathname: '/mypage', search: '' },
      writable: true,
      configurable: true,
    });
    ...
    (globalThis as any).fetch = vi.fn((url: string) => {
      if (url.includes('/routes.json')) {
        return Promise.resolve({
          ...
          json: () => Promise.resolve({
            success: true,
            data: { routes: [{ path: '/mypage', layout: 'mypage', auth_required: true }] },
          }),
        });
      }
    ...
    expect(preloadSpy).toHaveBeenCalledWith('user');
  });
});
