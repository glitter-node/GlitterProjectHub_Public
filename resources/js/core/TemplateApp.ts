...
export class TemplateApp {
    ...
    private shouldPreloadAuth(routesData: any, authType: AuthType): boolean {
        if (authType === 'admin') {
            return true;
        }
        ...
        const matchedRoute = routes.find((route: Route) => {
            ...
            const pattern = this.routePathToPattern(route.path);
            return pattern.test(pathname);
        });

        return matchedRoute?.auth_required === true;
    }

    private routePathToPattern(path: string): RegExp {
        if (path === '/') {
            return /^\/$/;
        }
    ...
    async init(): Promise<void> {
        try {
            ...
            // 1. 템플릿 엔진 초기화, ComponentRegistry, routes.json 로딩
            ...
            const [_, __, routesData, templateConfig] = await Promise.all([
            ...
            if (this.shouldPreloadAuth(routesData, authType)) {
                await authManager.preloadAuth(authType);
                logger.log('User info preloaded');
            } else {
                logger.log('User info preload skipped for public route');
            }
    ...
    private async handleRouteChange(route: Route): Promise<void> {
        ...
        try {
            ...
            if (webSocketSources.length > 0) {
                const subscribeWebSocketSources = async () => {
                    if (routeChangeId !== this.currentRouteChangeId) {
                        return;
                    }
                ...
                if (typeof window.requestIdleCallback === 'function') {
                    window.requestIdleCallback(() => {
                        void subscribeWebSocketSources();
                    }, { timeout: 1500 });
                } else {
