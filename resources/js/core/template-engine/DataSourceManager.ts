...
export class DataSourceManager {
    ...
    await Promise.all(
      autoFetchSources.map(async (source) => {
        try {
          ...
          const isSuppressedExpectedAuthError = errorCode === 401 && (
            (error as any)?._authSkipped === true ||
            source.errorHandling?.[401]?.handler === 'suppress' ||
            source.errorHandling?.['401']?.handler === 'suppress'
          );

          if (isSuppressedExpectedAuthError) {
            logger.log(`Data source auth state is anonymous: ${source.id}`);
            if (source.auth_required === true) {
              const apiClient = getApiClient() as any;
              if (typeof apiClient.removeToken === 'function') {
                apiClient.removeToken();
              }
              const authManager = AuthManager.getInstance() as any;
              if (typeof authManager.clearState === 'function') {
  ...
  private async fetchApiDataSource(
    ...
    try {
      ...
      // 토큰 유무 확인
      const authApiClient = getApiClient() as any;
      const apiToken = typeof authApiClient.getToken === 'function' ? authApiClient.getToken() : null;
      const hasToken = !!apiToken || AuthManager.getInstance().isAuthenticated();
