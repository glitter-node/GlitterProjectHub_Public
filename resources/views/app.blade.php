...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ $templateSeoMeta['title'] ?? config('app.name', '그누보드7') }}</title>
        @if(!empty($templateSeoMeta['description']))
        <meta name="description" content="{{ $templateSeoMeta['description'] }}">
        @endif
        @if(!empty($templateSeoMeta['title']))
        <meta property="og:title" content="{{ $templateSeoMeta['title'] }}">
        @endif
        @if(!empty($templateSeoMeta['description']))
        <meta property="og:description" content="{{ $templateSeoMeta['description'] }}">
        @endif
        <meta property="og:type" content="{{ $templateSeoMeta['og_type'] ?? 'website' }}">
        <meta property="og:url" content="{{ $templateSeoMeta['url'] ?? url()->current() }}">
        @if(!empty($templateSeoMeta['og_image']))
        <meta property="og:image" content="{{ $templateSeoMeta['og_image'] }}">
        @endif
        <meta name="twitter:card" content="{{ $templateSeoMeta['twitter_card'] ?? 'summary' }}">
        @if(!empty($templateSeoMeta['title']))
        <meta name="twitter:title" content="{{ $templateSeoMeta['title'] }}">
        @endif
        @if(!empty($templateSeoMeta['description']))
        <meta name="twitter:description" content="{{ $templateSeoMeta['description'] }}">
        @endif
        @if(!empty($templateSeoMeta['twitter_image']))
        <meta name="twitter:image" content="{{ $templateSeoMeta['twitter_image'] }}">
        @endif
        @if(!empty($templateHeadTags))
        {!! $templateHeadTags !!}
        @endif
