...
class DashboardService
{
    ...
    public function getSystemResources(): array
    {
        ...
        $resources = [
            'cpu' => $this->safeResourceProbe(fn () => $this->getCpuUsage(), [
                'percentage' => 0,
                'color' => $this->getResourceColor(0),
            ]),
            'memory' => $this->safeResourceProbe(fn () => $this->getMemoryUsage(), [
                'percentage' => 0,
                'used' => $this->formatBytes(0),
                'total' => $this->formatBytes(0),
                'color' => $this->getResourceColor(0),
            ]),
            'disk' => $this->safeResourceProbe(fn () => $this->getDiskUsage(), [
                'percentage' => 0,
                'used' => $this->formatBytes(0),
                'total' => $this->formatBytes(0),
                'color' => $this->getResourceColor(0),
    ...
    protected function getCpuUsage(): array
    {
        ...
        if (PHP_OS_FAMILY === 'Windows') {
            ...
            if (function_exists('sys_getloadavg')) {
                ...
                $cores = (int) $this->safeShellExec('nproc 2>/dev/null') ?: 1;
                $percentage = isset($load[0]) ? min(100, round(($load[0] / $cores) * 100)) : 0;
            }
        }
    ...
    private function getWindowsCpuUsage(): int
    {
        ...
        $output = $this->safeShellExec('powershell -NoProfile -Command "Get-CimInstance -ClassName Win32_Processor | Select-Object -ExpandProperty LoadPercentage" 2>&1');
        if ($output !== null) {
        ...
        $output = $this->safeShellExec('wmic cpu get loadpercentage 2>&1');
        if ($output) {
            $lines = explode("\n", trim($output));
            if (isset($lines[1])) {
    ...
    protected function getMemoryUsage(): array
    {
        ...
        if (PHP_OS_FAMILY === 'Windows') {
            ...
            if ($this->safeFileExists('/proc/meminfo')) {
                $memInfo = $this->safeFileGetContents('/proc/meminfo');
                if ($memInfo !== null && preg_match('/MemTotal:\s+(\d+)/', $memInfo, $matches)) {
                    $total = $matches[1] * 1024;
                }
                if ($memInfo !== null && preg_match('/MemAvailable:\s+(\d+)/', $memInfo, $matches)) {
    ...
    private function getWindowsMemoryUsage(): array
    {
        ...
        // PowerShell Get-CimInstance 시도 (Windows 10/11 권장)
        $totalOutput = $this->safeShellExec('powershell -NoProfile -Command "(Get-CimInstance -ClassName Win32_ComputerSystem).TotalPhysicalMemory" 2>&1');
        $freeOutput = $this->safeShellExec('powershell -NoProfile -Command "(Get-CimInstance -ClassName Win32_OperatingSystem).FreePhysicalMemory" 2>&1');
        ...
        // wmic fallback (이전 Windows 버전)
        $totalOutput = $this->safeShellExec('wmic computersystem get totalphysicalmemory 2>&1');
        $freeOutput = $this->safeShellExec('wmic os get freephysicalmemory 2>&1');
    ...
    protected function getDiskUsage(): array
    {
        ...
        foreach ($paths as $path) {
            $pathTotal = $this->safeDiskTotalSpace($path);
            $pathFree = $this->safeDiskFreeSpace($path);

            if ($pathTotal !== null && $pathFree !== null && $pathTotal > 0) {
                $total = $pathTotal;
                $free = $pathFree;
                break;
            }
    ...
    private function safeResourceProbe(callable $callback, array $fallback): array
    {
        try {
            return $callback();
        } catch (Throwable) {
    ...
    private function safeFileGetContents(string $path): ?string
    {
        try {
    ...
    private function safeDiskTotalSpace(string $path): ?float
    {
        try {
    ...
    private function safeDiskFreeSpace(string $path): ?float
    {
        try {
