...
class SettingsService
{
    ...
    private function getMemoryUsage(): array
    {
        ...
        if (PHP_OS_FAMILY === 'Linux' && $this->safeIsReadable('/proc/meminfo')) {
    ...
    private function getDiskUsage(): array
    {
        ...
        foreach ($paths as $path) {
            $total = $this->safeDiskTotalSpace($path);
            $free = $this->safeDiskFreeSpace($path);
    ...
    private function getCpuInfo(): string
    {
        ...
        if ($this->safeFileExists('/proc/cpuinfo')) {
            $cpuInfo = $this->safeFileGetContents('/proc/cpuinfo');
            if ($cpuInfo !== null && preg_match('/model name\s*:\s*(.+)/i', $cpuInfo, $matches)) {
