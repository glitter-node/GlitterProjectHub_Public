...
class AttachmentService
{
    ...
    public function upload(
        ...
        $this->lifecycle->logLifecycle('accepted', 'core', $attachment);
        FinalizeAttachmentUploadJob::dispatch('core', $attachment->id);
        $this->lifecycle->logLifecycle('queued', 'core', $attachment);

        Log::info('첨부파일 업로드 접수 완료', [
    ...
    public function download(string $hash, mixed $user = null): ?StreamedResponse
    {
        ...
        if (! $attachment->isReadyForServing()) {
            Log::warning('준비되지 않은 첨부파일 다운로드 차단', [
                'attachment_id' => $attachment->id,
    ...
    public function getFileInfo(string $hash, mixed $user = null): ?array
    {
        ...
        if (! $attachment->isReadyForServing()) {
            Log::warning('준비되지 않은 첨부파일 파일 정보 조회 차단', [
                'attachment_id' => $attachment->id,
                'status' => $attachment->status?->value ?? $attachment->status,
            ]);
