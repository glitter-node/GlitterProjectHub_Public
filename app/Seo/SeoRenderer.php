...
class SeoRenderer implements SeoRendererInterface
{
    ...
    public function render(Request $request): ?string
    {
        ...
        $routeStylesheetConfig = $seoTemplateConfig['route_stylesheets'][$layoutName] ?? null;
        $deferredStylesheets = [];
        if (is_array($routeStylesheetConfig)) {
            $routeStylesheets = $this->resolveTemplateStylesheetUrls($templateIdentifier, $routeStylesheetConfig['stylesheets'] ?? []);
            $deferredStylesheets = $this->resolveTemplateStylesheetUrls($templateIdentifier, $routeStylesheetConfig['deferred_stylesheets'] ?? []);
            $allStylesheets = array_merge($routeStylesheets, $configStylesheets);
        } else {
        ...
        $viewData = [
            ...
            'deferredStylesheets' => $deferredStylesheets,
            'extraHeadTags' => '',
            'headAssetTags' => $this->buildTemplateHeadTags($templateIdentifier),
            'extraBodyEnd' => '',
    ...
    private function buildTemplateHeadTags(string $templateIdentifier): string
    {
        $path = base_path("templates/{$templateIdentifier}/template.json");
        if (! is_file($path)) {
            return '';
        }

        $templateJson = json_decode((string) file_get_contents($path), true);
    ...
    private function resolveTemplateStylesheetUrls(string $templateIdentifier, array $paths): array
    {
        $urls = [];
        foreach ($paths as $path) {
            ...
            $servePath = preg_replace('#^dist/#', '', $path);
