...
class ValidLayoutStructure implements ValidationRule
{
    ...
    public function validate(string $attribute, mixed $value, Closure $fail): void
    {
        ...
        // slots 필드 검증 (존재하는 경우만)
        if (isset($value['slots']) && ! $this->validateSlots($value, $fail)) {
            return;
        }
    }
    ...
    private function validateRequiredFields(array $data, Closure $fail): bool
    {
        ...
        // Production bundled layouts may define root components, named slots, or
        // extend another layout only to override metadata/state.
        if (! isset($data['components']) && ! isset($data['slots']) && ! isset($data['extends'])) {
        ...
        if (isset($data['slots']) && ! is_array($data['slots'])) {
            $fail(__('validation.layout.slots_must_be_object'));
     ...
     * slots 객체 검증
     */
    private function validateSlots(array $data, Closure $fail): bool
    {
        foreach ($data['slots'] as $slotName => $components) {
            if (! is_array($components)) {
                $fail(__('validation.layout.slot_components_must_be_array', ['slot' => $slotName]));

                return false;
            }

            foreach ($components as $index => $component) {
                if (! $this->validateComponent($component, "slots.{$slotName}.{$index}", $fail)) {
                    return false;
                }
            }
        }
