...
class ValidLayoutStructureTest extends TestCase
{
    ...
    public function test_allows_extension_point_component_type_used_by_runtime_renderer(): void
    {
        $layout = [
            'version' => '1.0',
            'layout_name' => 'Extension Point Layout',
            'components' => [
                [
                    'id' => 'address_search_slot',
        ...
        $this->assertFalse($failed, 'Expected runtime-supported extension_point components to be allowed');
    }


    public function test_allows_extends_only_layout_override_used_by_bundled_sources(): void
    {
        $layout = [
            'version' => '1.0.0',
            'layout_name' => 'Child Override Layout',
            'extends' => 'base_layout',
            'state' => [
                'scopeContext' => 'all',
            ],
        ...
        $this->assertFalse($failed, 'Expected extends-only layout metadata/state overrides to be allowed');
    }

    public function test_allows_slots_only_layout_used_by_bundled_sources(): void
    {
        $layout = [
            'version' => '1.0.0',
            'layout_name' => 'Slot Only Layout',
            'slots' => [
                'content' => [
                    ['partial' => 'partials/example.json'],
                ],
            ],
        ];
        ...
        $this->assertFalse($failed, 'Expected slots-only bundled layout payloads to be allowed');
    }
