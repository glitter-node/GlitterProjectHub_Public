...
class AttachmentService
{
    ...
    public function upload(
        ...
        $uploadedMime = $file->getClientMimeType() ?: $file->getMimeType();
        ...
        $meta = $this->lifecycle->queuedProcessingMeta($meta);
        ...
        $attachment = $this->repository->create($slug, [
            ...
            'disk' => $disk,
            'path' => $path,
            'mime_type' => $uploadedMime ?: 'application/octet-stream',
            'size' => $file->getSize(),
            'status' => AttachmentStatus::Pending->value,
            'attachment_type' => AttachmentType::Unknown->value,
            'processed_at' => null,
            'failed_reason' => null,
        ...
        $this->lifecycle->logLifecycle('accepted', 'board', $attachment);
        FinalizeAttachmentUploadJob::dispatch('board', $attachment->id);
        $this->lifecycle->logLifecycle('queued', 'board', $attachment);

        Log::info('게시판 첨부파일 업로드 접수 완료', [
            'board_slug' => $slug,
    ...
    public function download(string $slug, int $id, string $context = 'admin'): ?StreamedResponse
    {
        ...
        if (! $attachment->isReadyForServing()) {
            Log::warning('준비되지 않은 게시판 첨부파일 다운로드 차단', [
                'attachment_id' => $attachment->id,
                'status' => $attachment->status?->value ?? $attachment->status,
            ]);
    ...
    public function preview(string $slug, int $id): ?StreamedResponse
    {
        ...
        if (! $attachment->isReadyForServing()) {
            Log::warning('준비되지 않은 게시판 첨부파일 미리보기 차단', [
                'attachment_id' => $attachment->id,
                'status' => $attachment->status?->value ?? $attachment->status,
            ]);
    ...
    public function getFileInfo(string $slug, int $id): ?array
    {
        ...
        if (! $attachment->isReadyForServing()) {
            Log::warning('준비되지 않은 게시판 첨부파일 파일 정보 조회 차단', [
                'attachment_id' => $attachment->id,
                'status' => $attachment->status?->value ?? $attachment->status,
            ]);
