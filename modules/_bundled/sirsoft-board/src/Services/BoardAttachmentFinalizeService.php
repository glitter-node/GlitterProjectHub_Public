...
class BoardAttachmentFinalizeService
{
    ...
    public function finalize(int $attachmentId, int $jobAttempt = 1): void
    {
        ...
        if (! $attachment) {
            Log::info('게시판 첨부파일 finalize 건너뜀: 행 없음', [
                'domain' => 'board',
        ...
        if ($status === AttachmentStatus::Processing) {
            if ($this->lifecycle->isProcessingStale($attachment)) {
                if ($this->lifecycle->markStaleFailed($attachment, 'board')) {
                    HookManager::doAction('sirsoft-board.attachment.after_failed', $attachment->fresh());
                }
            }
        ...
        if ($status === AttachmentStatus::Failed) {
            $this->lifecycle->logLifecycle('retry_started', 'board', $attachment, ['reason' => $attachment->failed_reason]);
        }
        ...
        if ($claimed !== 1) {
            Log::info('게시판 첨부파일 finalize 건너뜀: processing claim 실패', [
                'domain' => 'board',
                'attachment_id' => $attachmentId,
                'status' => $status->value,
            ]);
        ...
        $this->lifecycle->markStarted($attachment, 'board', $jobAttempt);
        ...
        if ($status === AttachmentStatus::Ready) {
            $this->lifecycle->fireCompleted('board', $attachment);
            HookManager::doAction('sirsoft-board.attachment.after_ready', $attachment);
        } else {
            $this->lifecycle->fireQuarantined('board', $attachment, $inspection['failed_reason']);
            HookManager::doAction('sirsoft-board.attachment.after_quarantined', $attachment);
        }
    }
     ...
     */
    private function mergeProcessingMeta(Attachment $attachment, array $inspection, string $storedFilePath): array
    {
    ...
    private function markFailed(Attachment $attachment, string $reason): void
    {
        ...
        $attachment = $attachment->fresh();
        $this->lifecycle->fireFailed('board', $attachment, $reason);
        HookManager::doAction('sirsoft-board.attachment.after_failed', $attachment);
    }
}
