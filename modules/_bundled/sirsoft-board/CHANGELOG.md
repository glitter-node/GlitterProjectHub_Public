...
## [1.0.0-beta.8] - 2026-05-20

### Changed

- 게시판 첨부파일 비동기 finalize 파이프라인에 stale processing 복구, retry-safe idempotency, 운영 메타데이터, quarantine 메타데이터, 구조화 lifecycle 로그, 공통 processing 훅을 추가

## [1.0.0-beta.7] - 2026-05-20

### Changed

- 게시판 첨부파일 업로드를 비동기 finalize 파이프라인으로 전환 — 요청 경로는 파일 저장과 pending row 접수만 수행하고 MIME 검사, 이미지 크기 추출, ready/quarantined/failed 전환은 큐 finalizer 에서 처리
- 게시판 첨부파일 finalize 훅 추가: `sirsoft-board.attachment.after_ready`, `sirsoft-board.attachment.after_quarantined`, `sirsoft-board.attachment.after_failed`
