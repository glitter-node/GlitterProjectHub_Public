...
class UpdatePageRequest extends FormRequest
{
    protected function prepareForValidation(): void
    {
        ...
        foreach (['title', 'description', 'keywords'] as $field) {
            $seoMeta[$field] = $this->normalizeSeoMetaValue($seoMeta[$field] ?? null);
        }
    ...
    private function normalizeSeoMetaValue(mixed $value): string
    {
        if (is_string($value)) {
            return $value;
        }
        ...
        foreach (['ko', 'en'] as $locale) {
            if (array_key_exists($locale, $value)) {
                $normalized = $this->normalizeSeoMetaValue($value[$locale]);
        ...
        foreach ($value as $item) {
            $normalized = $this->normalizeSeoMetaValue($item);
